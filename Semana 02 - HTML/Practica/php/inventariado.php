...
if(isset($_POST['accion'])){
	...
	switch ($_POST['accion']) {
		case 'read':
			...
			break;
		case 'llenarTabla':
			ActionLlenarPHP($conexion);
			break;
		case 'datosProducto':
			ActtionTenerDatos($conexion);
			break;
		case 'llenarTablaSucursal':
			llenarTablaSucursales($conexion);
			break;
...
function ActionReadPHP($conexion){
	...
	$Query 		="SELECT Nombre, SUM(stock) as Disponible, Precio,  IDProducto FROM productos Group By Nombre";	
	...
	while($Renglon = mysqli_fetch_array($Resultado)){
		...
		$Alta_Evento["cantidad"]		 = $Renglon["Disponible"];
		$Alta_Evento["precio"]		 = $Renglon["Precio"];
		$Alta_Evento["id"]		 = $Renglon["IDProducto"];
...
function ActionLlenarPHP($conexion){
	...
	if(isset($_POST['inventario']))
	{
		...
		$Query 		="SELECT Nombre, SUM(stock) as Disponible, Precio, IDProducto FROM productos where IDCategoria='".$Inventario."' Group By Nombre";	
		$Resultado 	= mysqli_query($conexion,$Query);
		$Respuesta["mensaje"]	="SELECT Nombre, SUM(stock) as Disponible, Precio, IDProducto FROM productos where IDCategoria='".$Inventario."' Group By Nombre";
		...
		{
		  $Respuesta["estado"]	=0;
		  $Respuesta["mensaje"]	="0 elementos en el inventario";
		}
	...
	{
		$Respuesta["estado"]	=0;
		$Respuesta["mensaje"]	="No se identifico el inventario";
	}
...
function ActtionTenerDatos($conexion)
{
	//$Respuesta["alta_eventos"]	=	array();
	...
	if(isset($_POST['id']))
	{
		...
		$Query 		="SELECT productos.Nombre as NombreM, categoria.Nombre as NombreC, Editorial FROM productos INNER JOIN categoria ON productos.IDCategoria = categoria.IDCategoria WHERE productos.Nombre = '".$NombreProducto."' Group By productos.Nombre";	
		$Resultado 	= mysqli_query($conexion,$Query);
		...
		if(mysqli_affected_rows($conexion)>0)
		{
				...
				//$Alta_Evento = array();
				$Respuesta["nombre"]		 = $Renglon["NombreM"];
				$Respuesta["categoria"]		 = $Renglon["NombreC"];
				$Respuesta["Editorial"]		 = $Renglon["Editorial"];
				$Respuesta["mensaje"]	="Correcto";
				$Respuesta["estado"]	=1;
		}
		else
	...
	{
		$Respuesta["estado"]	=0;
		$Respuesta["mensaje"]	="No se identifico el nombre";
	}

	echo json_encode($Respuesta);
}

function llenarTablaSucursales($conexion)
{
	$Respuesta["alta_eventos"]	=	array();
	...
	{
		$Inventario	= $_POST['id'];
		$Query 		="SELECT sucursal.IDSucursal, sucursal.Lugar, Stock FROM productos INNER JOIN sucursal ON productos.IDSucursal = sucursal.IDSucursal WHERE productos.Nombre = '".$Inventario."'";	
		
		$Resultado 	= mysqli_query($conexion,$Query);
		$Respuesta["mensaje"]	= "SELECT sucursal.IDSucursal, sucursal.Lugar, Stock FROM productos INNER JOIN sucursal ON productos.IDSucursal = sucursal.IDSucursal WHERE productos.Nombre = '".$Inventario."'";
		...
		if(mysqli_affected_rows($conexion)>0)
		{
			...
			while($Renglon = mysqli_fetch_array($Resultado)){
				...
				$Alta_Evento = array();
				$Alta_Evento["id"]		 = $Renglon["IDSucursal"];
				$Alta_Evento["lugar"]		 = $Renglon["Lugar"];
				$Alta_Evento["Stock"]		 = $Renglon["Stock"];
